export default class SpriteUtilities {
  ...
  filmstrip(
    ...
    const textureWidth = this.TextureCache[texture].width;


    const textureHeight = this.TextureCache[texture].height;

    // Find out how many columns and rows there are
    const columns = textureWidth / frameWidth;


    const rows = textureHeight / frameHeight;
  ...
  shoot(
    ...
    const tempGx = bullet.getGlobalPosition().x;


    const tempGy = bullet.getGlobalPosition().y;
