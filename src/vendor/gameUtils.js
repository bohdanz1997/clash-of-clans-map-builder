export default class GameUtilities {
  ...
  */
  ...
  distance(s1, s2) {
    const vx = (s2.x + this._getCenter(s2, s2.width, 'x')) - (s1.x + this._getCenter(s1, s1.width, 'x'));


    const vy = (s2.y + this._getCenter(s2, s2.height, 'y')) - (s1.y + this._getCenter(s1, s1.height, 'y'));
  ...
  */
  ...
  followEase(follower, leader, speed) {
    ...
    */
    ...
    const vx = (leader.x + this._getCenter(leader, leader.width, 'x')) - (follower.x + this._getCenter(follower, follower.width, 'x'));


    const vy = (leader.y + this._getCenter(leader, leader.height, 'y')) - (follower.y + this._getCenter(follower, follower.height, 'y'));
  ...
  */
  ...
  rotateAroundSprite(rotatingSprite, centerSprite, distance, angle) {
    rotatingSprite.x = (centerSprite.x + this._getCenter(centerSprite, centerSprite.width, 'x')) - rotatingSprite.parent.x
      + (distance * Math.cos(angle))
      - this._getCenter(rotatingSprite, rotatingSprite.width, 'x');

    rotatingSprite.y = (centerSprite.y + this._getCenter(centerSprite, centerSprite.height, 'y')) - rotatingSprite.parent.y
  ...
  */
  ...
  lineOfSight(
    ...
    const vy = spriteTwoCenterY - spriteOneCenterY;
    ...
    const points = () => {
      ...
      for (let i = 1; i <= numberOfPoints; i++) {
        ...
        const y = spriteOneCenterY + dy * newMagnitude;
    ...
    const hitTestPoint = (point, sprite) => {
      ...
      const right = point.x < (sprite.x + sprite.width);


      const top = point.y > sprite.y;


      const bottom = point.y < (sprite.y + sprite.height);
    ...
    const noObstacles = points().every(point => obstacles.every(obstacle => !(hitTestPoint(point, obstacle))));
